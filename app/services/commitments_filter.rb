  ...
  def process_request(commitments, sort, dir)
    if sort == @sort_by[:end_date]
      if dir == @sort_direction[:down]
        results = commitments.sort_end_date_down
      else
        results = commitments.sort_end_date_up
      end 
    elsif sort == @sort_by[:payment]
      if dir == @sort_direction[:down]
        results = commitments.sort_payment_down
      else
        results = commitments.sort_payment_down
      end 
    elsif sort == @sort_by[:organization]
      if dir == @sort_direction[:down]
        results = commitments.sort_organization_down
      else
        results = commitments.sort_organization_up
      end 
    else 
      # default sort is by start_date
      if dir == @sort_direction[:down]
        results = commitments.sort_start_date_down
      else
        results = commitments.sort_start_date_up
      end
    end  
    return results
  end
