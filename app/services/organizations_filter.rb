  ...
  def process_request(orgs, query, sort, dir)
    ...
    if sort == @sort_by[:contacts]
      if dir == @sort_direction[:down]
        organizations = orgs.sort_contact_down
      else
        organizations = orgs.sort_contact_up
      end 
    elsif sort == @sort_by[:types]
      if dir == @sort_direction[:down]
        organizations = orgs.sort_type_down
      else
        organizations = orgs.sort_type_up
      end 
    else 
      # default sort is by name
      if dir == @sort_direction[:down]
        organizations = orgs.sort_name_down
      else
        organizations = orgs.sort_name_up
      end
    end  
